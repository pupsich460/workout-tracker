...
class TestExercises:
    ...
    async def test_create_exercise_unauthorized(self, client):
        ...
        response = await client.post("/exercises/", json={"name": "Тест", "description": ""})
    ...
    async def test_get_exercise_by_id(self, client, auth_headers, exercise):
        ...
        response = await client.get(f"/exercises/{exercise['id']}", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_update_exercise(self, client, auth_headers, exercise):
        ...
        response = await client.patch(f"/exercises/{exercise['id']}", json={
    ...
    async def test_update_exercise_duplicate_name(self, client, auth_headers):
        ...
        await client.post("/exercises/", json={"name": "Упражнение А", "description": ""}, headers=auth_headers)
        ex_b = await client.post("/exercises/", json={"name": "Упражнение Б", "description": ""}, headers=auth_headers)

        # Пытаемся переименовать Б в А
        response = await client.patch(f"/exercises/{ex_b.json()['id']}", json={
            "name": "Упражнение А",
        }, headers=auth_headers)
        assert response.status_code == 400
    ...
    async def test_update_exercise_not_found(self, client, auth_headers):
        ...
        response = await client.patch("/exercises/999", json={"name": "X"}, headers=auth_headers)
        assert response.status_code == 404

    async def test_delete_exercise(self, client, auth_headers, exercise):
        """Удаление упражнения возвращает 204."""
        response = await client.delete(f"/exercises/{exercise['id']}", headers=auth_headers)
        assert response.status_code == 204

        get_response = await client.get(f"/exercises/{exercise['id']}", headers=auth_headers)
    ...
    async def test_other_user_cannot_see_exercise(self, client, exercise, auth_headers):
        ...
        response = await client.get(f"/exercises/{exercise['id']}", headers=other_headers)
        assert response.status_code == 404
