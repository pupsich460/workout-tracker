...
class TestWorkouts:
    ...
    async def test_update_workout(self, client, auth_headers, workout):
        ...
        response = await client.patch(f"/workouts/{workout['id']}", json={
    ...
    async def test_update_workout_not_found(self, client, auth_headers):
        ...
        response = await client.patch("/workouts/999", json={"name": "X"}, headers=auth_headers)
        assert response.status_code == 404
    ...
    async def test_delete_workout(self, client, auth_headers, workout):
        ...
        get_response = await client.get(f"/workouts/{workout['id']}", headers=auth_headers)
    ...
    async def test_other_user_cannot_access_workout(self, client, workout, auth_headers):
...
class TestWorkoutExercises:
    ...
    async def test_add_exercise_to_workout(self, client, auth_headers, workout, exercise):
        """Добавление упражнения в тренировку возвращает 201."""
        response = await client.post(f"/workouts/{workout['id']}/exercises", json={
    ...
    async def test_add_exercise_to_nonexistent_workout(self, client, auth_headers, exercise):
        """Добавление упражнения в несуществующую тренировку возвращает 404."""
        response = await client.post("/workouts/999/exercises", json={
            "workout_id": 999,
            "exercise_id": exercise["id"],
            "sets": 3,
            "reps": 10,
    ...
    async def test_get_workout_exercises(self, client, auth_headers, workout, exercise):
        ...
        await client.post(f"/workouts/{workout['id']}/exercises", json={
            "workout_id": workout["id"],
            "exercise_id": exercise["id"],
            "sets": 3,
            "reps": 10,
        ...
        response = await client.get(f"/workouts/{workout['id']}/exercises", headers=auth_headers)
        assert response.status_code == 200
        assert len(response.json()) == 1
